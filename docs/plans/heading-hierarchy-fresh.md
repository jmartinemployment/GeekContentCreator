...
Heading hierarchy should be visible after Site Analyzer finishes analyzing a domain — before gap selection.

**Verified:** `HeadingDto(int Level, string Text)` already exists in `GccResearchModels.cs:56`. `RelatedPageDto` already carries `HeadingDto[] Headings`. Data path exists: `snapshot.SitePages` in `GccController.cs:1168` has full heading trees.

Content Creator always forces fresh analysis (`force: true` in `site-analyzer-client.tsx:148`), so only the fresh-completion path (lines 1161–1199 in `GccController.cs`) is used.

## Backend Change

**File:** `GeekBackend/GeekAPI/Controllers/ContentCreator/GccController.cs` (lines 1161–1199)
...
Modify return statement (lines 1191–1199) to add `pages`:
```csharp
return Ok(new
{
    analysis.Id,
...
3. In `pollUntilDone` (line 109–110), add after `setGaps`:
   ```typescript
...
- Live: Analyze domain → after "ready" → "Site structure" collapsible appears above gaps → expand to see all pages and h1–h6 trees
