...
**Fixed:** content-gap detection previously fabricated gaps — a pillar with <3 real crawled child pages was padded with 5 hardcoded generic subtopic templates instead of real findings. Now replaced with real heading-based gap detection (`SiteContentCoverageMatcher.HasNoMatchingPage`/`UrlBelongsToPillarSlug`, headings extracted per-crawled-page inline in `RunSiteCrawlAsync`). This shipped alongside a broader elimination of silent fallback/soft-success patterns across Geek-SEO and GeekAPI (provider auto-switching, swallowed exceptions, artifact-blob substitution) — see [docs/FALLBACK_ELIMINATION_PLAN.md](./docs/FALLBACK_ELIMINATION_PLAN.md) ([inventory](./docs/FALLBACK_INVENTORY.md)). 195/195 Geek-SEO tests pass; not yet live-verified against a real domain.
